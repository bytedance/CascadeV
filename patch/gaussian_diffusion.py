# Modified from https://github.com/PixArt-alpha/PixArt-sigma/blob/master/diffusion/model/gaussian_diffusion.py
import enum
import math
import numpy as np
import torch as th
...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, x_ref, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        """
        ...
        if x_ref is not None:
            model_output = model(th.cat([x, x_ref], dim=1), t, **model_kwargs)
        else:
    ...
    def _vb_terms_bpd(
            self, model, x_start, x_t, x_ref, t, clip_denoised=True, model_kwargs=None
    ):
        """
        ...
        out = self.p_mean_variance(
            model, x_t, x_ref, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )
        kl = normal_kl(
    ...
    def training_losses(self, model, x_start, x_ref, timestep, model_kwargs=None, noise=None, skip_noise=False):
        """
        ...
        if model_kwargs is None:
            model_kwargs = {}
        assert not skip_noise, 'skip_noise'
        if skip_noise:
            x_t = x_start
        else:
            if noise is None:
        ...
        terms = {}
        assert self.loss_type == LossType.MSE, self.loss_type
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            assert False
        elif self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            if x_ref is not None:
                model_output = model(th.cat([x_t, x_ref], dim=1), t, **model_kwargs)
            else:
