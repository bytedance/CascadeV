# Modified from https://github.com/PixArt-alpha/PixArt-sigma/blob/master/diffusion/model/dpm_solver.py
import torch
...
def model_wrapper(
    ...
    def noise_pred_fn(x, x_ref, t_continuous, cond=None):
        t_input = get_model_input_time(t_continuous)
        assert cond is None and x_ref is not None
        output = model(torch.cat([x, x_ref], dim=1), t_input)
        if model_type == "noise":
            return output
    ...
    def model_fn(x, x_ref, t_continuous):
        """
        The noise predicition model function that is used for DPM-Solver.
        """
        assert guidance_type == 'classifier-free', guidance_type
        if guidance_type == "uncond":
            ...
            if guidance_scale == 1. or unconditional_condition is None:
                return noise_pred_fn(x, x_ref, t_continuous, cond=condition)
            else:
                x_in = torch.cat([x] * 2)
                x_ref_in = torch.cat([x_ref] * 2) if x_ref is not None else None
                t_in = torch.cat([t_continuous] * 2)
                c_in = torch.cat([unconditional_condition, condition])
                noise_uncond, noise = noise_pred_fn(x_in, x_ref_in, t_in, cond=c_in).chunk(2)
...
class DPM_Solver:
    def __init__(
        ...
        """
        self.model = lambda x, x_ref, t: model_fn(x, x_ref, t.expand((x.shape[0])))
    ...
    def noise_prediction_fn(self, x, x_ref, t):
        """
        Return the noise prediction model.
        """
        return self.model(x, x_ref, t)

    def data_prediction_fn(self, x, x_ref, t):
        """
        Return the data prediction model (with corrector).
        """
        noise = self.noise_prediction_fn(x, x_ref, t)
    ...
    def model_fn(self, x, x_ref, t):
        """
        Convert the model to the noise prediction model or the data prediction model.
        """

        if self.algorithm_type == "dpmsolver++":
            return self.data_prediction_fn(x, x_ref, t)
        else:
            assert False
    ...
    def sample(self, x, x_ref, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
               method='multistep', lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver',
               atol=0.0078, rtol=0.05, return_intermediate=False,
               ):
        """
        ...
        """
        assert not denoise_to_zero, denoise_to_zero
        assert method == 'multistep', method
        ...
        assert self.correcting_xt_fn is None
        assert self.algorithm_type == "dpmsolver++", self.algorithm_type
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                t = timesteps[step]
                t_prev_list = [t]
                model_prev_list = [self.model_fn(x, x_ref, t)]

                assert self.correcting_xt_fn is None, self.correcting_xt_fn
                if self.correcting_xt_fn is not None:
                ...
                for step in range(1, order):
                    ...
                    model_prev_list.append(self.model_fn(x, x_ref, t))
                ...
                for step in tqdm(range(order, steps + 1)):
                    ...
                    if step < steps:
                        model_prev_list[-1] = self.model_fn(x, x_ref, t)
